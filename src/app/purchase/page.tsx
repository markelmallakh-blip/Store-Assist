...
function Purchase() {
  ...
  const { data: catalog } = useApi<{ items: CatalogItem[] }>("/api/catalog");
  ...
  const [lines, setLines] = useState<Line[]>(() => {
    ...
    if (!variant) return [];
    return [{ key: newKey(), text: null, qty: Number(params.get("qty")) || 1, unitCost: "", candidates: [], selected: variant, searching: false }];
  });
  ...
  async function addFiles(files: FileList | null) {
    ...
    try {
      ...
      setError("Couldn't open that photo. Try taking it again or use a JPG/PNG.");
    }
  }
  ...
  return (
    <>
      ...
      <Card className="mb-4 p-4">
        <input ref={cameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => addFiles(e.target.files).then(() => (e.target.value = ""))} />
        <input ref={galleryRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => addFiles(e.target.files).then(() => (e.target.value = ""))} />
        ...
        <div className="mt-4 border-t border-line pt-3">
          ...
          <ProductPicker
            ...
            onPick={(item) =>
              setLines((ls) => [
                ...ls,
                { key: newKey(), text: null, qty: 1, unitCost: (item as CatalogItem).cost ? String((item as CatalogItem).cost) : "", candidates: [], selected: item.id, searching: false },
              ])
            }
          />
        </div>
    ...
    <Card className={`p-3.5 ${needsChoice ? "ring-2 ring-amber/50" : ""}`}>
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1">
          {line.text && <div className="text-xs text-fg-muted">On the photo: “{line.text}”</div>}
          {item && !line.searching ? (
